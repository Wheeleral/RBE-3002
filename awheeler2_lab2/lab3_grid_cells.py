...
def color(x,y)
    #print "here"
    pubColor = rospy.Publisher("/grid/cells", GridCells, queue_size=1)  
    print "Coloring"
    cellmsg =GridCells()
    cellmsg.header.frame_id = 'map'
    cellmsg.cell_height =resolution
    cellmsg.cell_width =resolution
    point = Point()
    point.x= x
    point.y =y
    point.z =0

    cellmsg.cells.append(point)
    pubColor.publish(cellmsg)
